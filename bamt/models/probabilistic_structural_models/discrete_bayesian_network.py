...
class DiscreteBayesianNetwork(BayesianNetwork):
    """
    Bayesian Network for discrete variables using the 2.0.0 architecture.
    ...
    def fit(self, data):
        ...
        from ...core.node_models import EmpiricalDistribution
        ...
        for node in self.nodes:
            ...
            if len(parents) == 0:
                # Root node - fit empirical distribution
                ...
                # Child node - fit conditional probability table
                # For simplicity, use empirical distribution (in full impl, use CPT)
                dist = EmpiricalDistribution()
                dist.fit(data[node].values)
                self.node_models[node] = {
                    'type': 'child',
    ...
    def predict(self, evidence, target: Optional[List[str]] = None):
        ...
        # Simple prediction: use mode of fitted distribution
        predictions = {}
        for node in target:
    ...
    def sample(self, num_samples: int):
        ...
        for node in topo_order:
            if node in self.node_models:
                ...
                # Not fitted yet - sample randomly
                samples[node] = np.random.choice(['A', 'B'], num_samples)
                
        return pd.DataFrame(samples)

    def __str__(self):
        return f"Discrete Bayesian Network (2.0) with {len(self.nodes)} nodes and {len(self.edges)} edges"

"""
Continuous Bayesian Network implementation.

This module implements a Bayesian Network for continuous (numerical) data.
"""
...
class ContinuousBayesianNetwork(BayesianNetwork):
    ...
    A Bayesian Network for modeling continuous/numerical data.
    Nodes use Gaussian distributions (root nodes) or regression
    ...
    def __init__(self, structure: Optional[nx.DiGraph] = None):
        """
        Initialize Continuous Bayesian Network.
    ...
    def fit(
        ...
        **kwargs
        ...
        """
        Fit the Continuous Bayesian Network.

        Args:
            data: Training data (continuous/numerical columns)
        ...
        for node_name in self.structure.nodes():
            ...
            if not parents:
                ...
                # For continuous BN, all parents are continuous
                node = ConditionalContinuousNode(
                    name=node_name,
                    disc_parents=[],
                    cont_parents=parents,
                )
    ...
    def sample(
        self, n_samples: int = 1, evidence: Optional[Dict[str, float]] = None, **kwargs
    ) -> pd.DataFrame:
        """
    ...
    def __str__(self):
        ...
        return f"ContinuousBayesianNetwork({n_nodes} nodes, {n_edges} edges, {status})"

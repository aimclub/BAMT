...
def custom_crossover_all_model(
        graph_first: CompositeModel,
        graph_second: CompositeModel,
        max_depth):
    num_cros = 100
    try:
        for _ in range(num_cros):
            ...
            selected_node2 = graph_second.get_nodes_by_name(str(selected_node1))[
                0]
    ...
    try:
        for _ in range(num_mut):
            ...
            if nodes_not_cycling:
                ...
                ml_models = MlModels()
                other_random_node.content['parent_model'] = ml_models.get_model_by_children_type(
                    other_random_node)
                break
...
def custom_mutation_add_model(graph: CompositeModel, **kwargs):
    try:
        ...
        nodes_with_parents = [node for node in all_nodes if (
            node.nodes_from != [] and node.nodes_from is not None)]
...
def composite_metric(graph: CompositeModel, data: pd.DataFrame, percent=0.02):
    ...
    data_train, data_test = train_test_split(
        data_all, train_size=0.8, random_state=42)
    ...
    for node in graph.nodes:
        ...
        if node.nodes_from is None or node.nodes_from == []:
            if node.content['type'] == 'cont':
                ...
                score += norm.logpdf(data_of_node_test.values,
                                     loc=mu, scale=sigma).sum()
            else:
            ...
            model, columns, target, idx = MlModels.dict_models[node.content['parent_model']](), [
                n.content['name'] for n in node.nodes_from], data_of_node_train.to_numpy(), data_train.index.to_numpy()
            ...
            if node.content['type'] == 'cont':
                ...
                mse = mean_squared_error(
                    target, predict, squared=False) + 0.0000001

...
class AICScore(ScoreFunction):
    """
    AIC (Akaike Information Criterion) score function for Bayesian networks.
    AIC = log-likelihood - k
    where k is the number of parameters.
    
    Lower AIC is better (penalizes model complexity less than BIC).
    """
    ...
    def compute(self, data, edges: List[Tuple[str, str]]) -> float:
        """
        Compute AIC score for a given DAG structure.
        ...
        Returns:
            float: AIC score (lower is better, typically negative)
        """
        if pd is None or np is None:
            raise ImportError("pandas and numpy required for AICScore")
        ...
        # AIC = log-likelihood - k
        aic = log_likelihood - num_parameters
